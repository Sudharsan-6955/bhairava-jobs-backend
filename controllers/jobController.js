...
const getJobById = asyncHandler(async (req, res) => {
  const isAdminUser = req.admin && (req.admin.role === 'admin' || req.admin.role === 'superadmin');
  const jobQuery = Job.findById(req.params.id).populate('createdBy', 'name email');

  if (isAdminUser) {
    jobQuery.setOptions({ includeDeleted: true });
  }
...
const updateJob = asyncHandler(async (req, res) => {
  ...
  // Reactivating a soft-deleted job should make it visible again.
  if (req.body.status === 'active' && job.isDeleted === true) {
